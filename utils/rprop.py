...
class RPropOptimizer(optimizer.Optimizer):
    ...
    def _create_slots(self, var_list):
        ...
        for v in var_list:
            self._get_or_make_slot(v, tf.fill(v.get_shape(), self._stepsize),
                                   "stepsize", self._name)
            self._get_or_make_slot(
                v, tf.zeros([v.get_shape().num_elements()], dtype=tf.float32),
                "stepmul", self._name)
            self._get_or_make_slot(
                v, tf.zeros([v.get_shape().num_elements()], dtype=tf.float32),
                "delta", self._name)
    ...
    def _apply_dense(self, grad, var):
        ...
        eta_plus_update = tf.cast(one_indices,
                                  tf.float32) * 0.0 + self._etaplus
        ...
        stepmul_up = tf.scatter_update(stepmul_up, m_one_indices,
                                       eta_minus_update)
